...
class NikeScraperOrchestrator {
    ...
    private NikeProductDetailScraper $productDetailScraper;
    private Website                  $website;

    public function __construct (
        NikeCategoryScraper $categoryScraper, NikeProductDetailScraper $productDetailScraper, Website $website
    ) {
    ...
    public function scrapeCategory (
        ?string $apiUrl = null, ?ScrapeJob $job = null, int $categoryConcurrency = 5, int $pdpConcurrency = 20, int $batchSize = 200, ?int $maxProducts = null, float $batchSleep = 0.2
    ) : void {
        ...
        try {
            ...
            Log::info('Starting Nike category scrape', [
            ...
            Log::info('Nike category scrape completed successfully', [
                'api_url'                 => $apiUrl ?? 'default',
                'total_products'          => $totalUrls,
            ...
            Log::error('Nike category scrape failed', [
                'api_url' => $apiUrl ?? 'default',
                'error'   => $e->getMessage(),
            ]);
