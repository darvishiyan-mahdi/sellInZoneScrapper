...
class CalvinKleinProductDetailScraper {
    private const BASE_URL                  = 'https://www.calvinklein.nl';
     ...
     *
     * @return string|null Relative path to saved image (e.g., 'images/ck_12_tanga_white_1702562345.jpg') or null on failure
     */
    private function downloadAndSaveImage (string $imageUrl, Product $product, string $slug, int $imageIndex) : ?string {
        try {
            ...
            // Generate filename: ck_{product_id}_{slug}_{variant}_{timestamp}.jpg
    ...
    private function checkCachedImage (int $productId, string $slug, int $imageIndex, string $imageUrl) : ?string {
        try {
            ...
            $pattern     = "ck_{$productId}_{$cleanSlug}{$variantPart}_*.jpg";

            // Search for existing files in images directory
            ...
            foreach ( $files as $file ) {
                ...
                if ( preg_match('/^ck_' . preg_quote($productIdStr, '/') . '_' . preg_quote($cleanSlug, '/') . preg_quote($variantPart, '/') . '_\d+\.jpg$/', $basename) ) {
    ...
    /**
     * Generate image filename following pattern: ck_{product_id}_{slug}_{variant}_{timestamp}.jpg
     *
     * @param int    $productId Product ID
     * @param string $slug      Product slug
    ...
    private function generateImageFilename (int $productId, string $slug, $variant, int $timestamp, string $extension = 'jpg') : string {
        ...
        return "ck_{$productId}_{$cleanSlug}{$variantPart}_{$timestamp}.{$extension}";
    }
     ...
     * @param int     $imageIndex Image index for variant identification
     *
     * @return string|null Relative path to saved image (e.g., 'images/ck_12_tanga_white_0_1702562345.jpg') or null on failure
     */
    private function downloadAndSaveVariantImage (string $imageUrl, Product $product, string $slug, string $colourSlug, int $imageIndex) : ?string {
        try {
            ...
            // Generate filename: ck_{product_id}_{slug}_{colourSlug}_{imageIndex}_{timestamp}.jpg
            $timestamp = time();
    ...
    /**
     * Generate variant image filename following pattern: ck_{product_id}_{slug}_{colourSlug}_{imageIndex}_{timestamp}.jpg
     *
     * @param int    $productId  Product ID
     * @param string $slug       Product slug
    ...
    private function generateVariantImageFilename (int $productId, string $slug, string $colourSlug, int $imageIndex, int $timestamp, string $extension = 'jpg') : string {
        ...
        return "ck_{$productId}_{$cleanSlug}_{$cleanColourSlug}_{$imageIndex}_{$timestamp}.{$extension}";
    }
}
