...
class LululemonScraperOrchestrator {
    private LululemonCategoryScraper      $categoryScraper;
    private LululemonProductDetailScraper $productDetailScraper;
    private Website                       $website;

    public function __construct (
        LululemonCategoryScraper $categoryScraper, LululemonProductDetailScraper $productDetailScraper, Website $website
    ) {
    ...
    public static function ensureWebsite () : Website {
        ...
        if ( !$website ) {
            $trashedWebsite = Website::withTrashed()->where('slug', 'lululemon')->first();
            ...
            if ( $trashedWebsite ) {
                ...
                $website = Website::create([
                    'name'          => 'Lululemon',
                    'base_url'      => 'https://www.eu.lululemon.com',
                    'slug'          => 'lululemon',
                    'is_active'     => true,
    ...
    public function scrapeCategory (
        ...
        try {
            ...
            $productUrls = $this->categoryScraper->loadProductLinksFromCache($categoryUrl);
            
            if ($productUrls === null) {
                Log::info('Step 1: Collecting product URLs from category page', [
                    'category_url' => $categoryUrl,
                ]);

                $productUrls = $this->categoryScraper->collectProductLinks($categoryUrl);
            } else {
                Log::info('Step 1: Using cached product URLs', [
                    'category_url' => $categoryUrl,
                    'cached_count' => count($productUrls),
                ]);
            }
            ...
            Log::info('Lululemon category scrape completed successfully', [
                'category_url'                 => $categoryUrl,
                'total_products'               => $totalUrls,
            ...
            Log::error('Lululemon category scrape failed', [
                'category_url' => $categoryUrl,
                'error'        => $e->getMessage(),
            ]);
